...
| 弱网反馈先解决等待不确定性 | PR26 只在 AI 请求超过 8 秒后切换慢响应文案，不改请求生命周期、重试状态或抓帧压缩，降低回归风险 |
| 弱网抓帧先压体积再追求原始分辨率 | PR27 将抓帧改为长边缩放 + JPEG 0.5 至 0.4 自适应尝试，优先获得约 40KB 内的图片，降低蜂窝网络上传等待 |
| 失败恢复要说明系统已经保留输入 | PR28 在 AI 请求失败且问题/画面仍可重试时，明确提示“已保留当前问题和画面”，让用户不必重新说话或重新抓帧 |
| 文本输入应是语音 fallback，而不是第二套对话系统 | PR29 在当前步骤卡片内加入文本问题输入，继续复用 `questionText`、抓帧、`askVisionQuestion` 和既有重试逻辑 |
