...
| PR30 | P3 回答摘要和复制：浏览器端提取摘要并支持复制摘要/全文 | PR29 |
| PR31 | P3 低清/高清抓帧模式：让用户在速度和准确度之间切换 | PR30 |
| PR32 | 文档：端到端真机测试清单，覆盖 iOS Safari 权限和 ngrok 隧道 | PR31 |
...
| 回答摘要不额外调用 AI | 摘要用于扫读和复制，先用浏览器端首句/截断规则生成，避免为 P3 辅助功能增加模型成本和等待时间 |
| 抓帧模式显式暴露速度/准确度取舍 | 默认低清延续约 40KB 上传策略；高清放宽体积保留细节，切换模式时清空旧帧避免发送与当前模式不一致的画面 |
| 真机测试清单单独成文 | 不改动已有设计文档，新增独立清单覆盖 iOS Safari、ngrok、权限、弱网和新增兜底功能，便于赛前复核 |
| 过程文档统一归档到 `docs/` | 根目录只保留入口级说明和配置，`docs/task_plan.md`、`docs/findings.md`、`docs/progress.md` 集中承载规划、发现和进度记录 |
